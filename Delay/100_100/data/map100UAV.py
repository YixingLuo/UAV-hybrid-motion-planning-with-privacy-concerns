...
A=np.loadtxt('occ_grid_height.txt',delimiter=' ')
# print("打印带高度信息的地图\n")
# print(A)

Label = np.loadtxt('ground.txt',delimiter=' ')

A1 = np.zeros((100,100),dtype=int)
#A2 = copy.deepcopy(A1)
A2 = copy.deepcopy(A1)
A3 = copy.deepcopy(A1)
A4 = copy.deepcopy(A1)
...
if __name__ == '__main__' :
    ...
    for num in range(220,221):
        ...
        mlab.points3d(0, 0, 0, mode='cube', color=(1, 1, 0), scale_mode='none', scale_factor='0.5')
        mlab.points3d(99, 99, 0, mode='cube', color=(1, 1, 0), scale_mode='none', scale_factor='0.5')
        ...
        # mlab.plot3d(x, y, z, color=(0.23, 0.6, 1), colormap='Spectral')
        mlab.plot3d(x1, y1, z1, color=(0, 0, 1), opacity=1, tube_radius=None, colormap='Spectral', line_width = 3)
        mlab.plot3d(x, y, z, color=(0, 0, 0), opacity=1, tube_radius=None, colormap='Spectral', line_width = 3)
        ...
        for i in range(np.shape(ccc)[0]):
            ...
            Z = camera_path_1[:, 0]
            for i in range(np.shape(ccc)[0]):
                mlab.plot3d(X, Y, Z, color=(0, 1, 0), opacity=1, tube_radius=None, colormap='Spectral', line_width = 3)

        a = plan_path_Hybrid[num][0]
        b = plan_path_Hybrid[num][1]
        c = plan_path_Hybrid[num][2]
        # print(c, a, b)
        # mlab.points3d(b, c, a, mode='sphere', color=(1, 1, 0), scale_mode='none', scale_factor='1')

        mlab.show()
        # mlab.savefig(os.getcwd() + '/pic/' + str(num)+'.png', figure=mlab.gcf(), magnification=3)