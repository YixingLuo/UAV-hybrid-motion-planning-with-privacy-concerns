...
#
#
# temp = np.load(os.getcwd()+'/data3/'+'plan_path_Hybrid_temp25.npy')
# print (temp)
...
# reference_path = "reference_path5" + ".npy"
...
if __name__ == '__main__' :
    ...
    for num in range(5):
        ...
        # 将所有建筑物可视化过程
        # A = np.loadtxt(os.getcwd() + "/data3/occ_grid_height" + str(num) + ".txt", delimiter=' ')
        A = np.loadtxt(os.getcwd() + "/data3/occ_grid_height" + str(0) + ".txt", delimiter=' ')
        # print("打印带高度信息的地图\n")
        print("height",A)

        # Label = np.loadtxt(os.getcwd() + "/data3/ground" + str(num) + ".txt", delimiter=' ')
        Label = np.loadtxt(os.getcwd() + "/data3/ground" + str(0) + ".txt", delimiter=' ')
        print("label", Label)
        A1 = np.zeros((A.shape[0], A.shape[1]), dtype=int)
        # A2 = copy.deepcopy(A1)
        A2 = copy.deepcopy(A1)
        ...
        plan_path_Hybrid1 = os.getcwd() + "/reference_path2" + ".npy"
        # plan_path_Hybrid1 = "plan_path_PP_temp" + str(num) + ".npy"
        plan_path_Hybrid = np.load(file=plan_path_Hybrid1)
        # print("综合规划路径数组类型输出：",plan_path_Hybrid.shape)
        # print("输出Hybird路径：\n")
        ...
        # camera状态改变可视化过程
        # camera_path = os.getcwd() + "/data3/plan_path_Hybrid_temp_2_" + str(num) + ".npy"
        camera_path = os.getcwd() + "/reference_path2" + ".npy"
        # camera_path = "plan_path_PP_temp" + str(num) + ".npy"
        camera_path = np.load(file=camera_path)
