...
def Astar_Hybrid_Planning_online(config, log, occ_grid_known, reference, occ_grid, socket, colorflag, sizeflag, continue_time, time_change):
    ...
    while not (idx >= len(trajectory_plan)):
        ...
        if idx == time_change:
            print("\033[95m change reference path! \033[0m")
            time.sleep(8)
            # T_plan = 26 - (idx+1)
            # T_plan_optimal = 20 - (idx+1)
            # replantime += 1
            # threat_list = []
            # next_p = trajectory_plan[-1]
            # aStar_pp = AStar(occ_grid_known, grid, current_p, next_p, T_plan, threat_list, 0, T_plan_optimal,
            #                  privacy_radius)
            # trajectory_optimal = aStar_pp.start()
            # for m in range(len(trajectory_optimal)):
            #     print("optimal， The No.", m, " step: ", trajectory_optimal[m])
            # first_part = trajectory_plan[0:idx + 1]
            # now_trajectory = first_part + trajectory_optimal
            # trajectory_plan = copy.deepcopy(now_trajectory)
            #
            # for m in range(len(trajectory_plan)):
            #     print("plan， The No.", m, " step: ", trajectory_plan[m])

            reference = np.load('data_raw/reference_path3_2.npy')
            trajectory_ref_temp = reference
            trajectory_ref = []
            for i in range(len(trajectory_ref_temp)):
