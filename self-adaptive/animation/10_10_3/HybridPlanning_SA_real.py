...
def Astar_Hybrid_Planning_online(config, log, occ_grid_known, reference, occ_grid, socket):
    ...
    while not (idx >= len(trajectory_plan)):
        ...
        if current_ca == 2: ## 1 = camera is open with high resolution, 2 = camera is off with lower resolution
            ...
            move a step
            """
            flight_control(idx, idx + 1, trajectory_plan, s)
        ...
        # take picture
        flag, occ_grid_known = hasprivacythreat_real(current_p, occ_grid_known, config, idx)

        # 简化实验
        # if idx == 5 or idx == 9:
        #     flag = 1
        #     occ_grid_known[0][3][3] = 3
        # if idx == 14:
        #     flag = 1
        #     occ_grid_known[0][6][6] = 4
        ...
        if flag:
            ...
            while start <= len(trajectory_plan)-1 :
                affect_path = []
                ...
                if (caculate_privacy_surround(grid, point, occ_grid_known, privacy_radius)) > 0:
                    ...
                    if T_plan >= distance:
                        replantime += 1
                        aStar_pp = AStar(occ_grid, grid, current_p, next_p, T_plan, threat_list, 0, T_plan_optimal, privacy_radius)
                        ...
                        if no_solution_flag > 0:
                            ...
                            log.info(
                                ...
                                % (
                                    ...
                                    no_solution_flag,
                                    PR_temp_sum_known, length_PP, T_plan_optimal))
                            aStar = AStar(occ_grid, grid, current_p, next_p, T_plan, threat_list, 1, T_plan_optimal, privacy_radius)
                        ...
                        if trajectory_optimal != None:
                            ...
                            # for m in range(len(trajectory_plan)):
                            #     print("plan， The No.", m, " step: ", trajectory_plan[m])
                        ...
                        print("not enough time!", T_plan, T_plan_optimal)
        ...
        #             print("producing local planning")
        #             replantime += 1
        #             aStar_pp = AStar(occ_grid,  grid,  current_p, next_p,
        #                              T_plan, threat_list, 0, T_plan_optimal, privacy_radius)
        #             trajectory_optimal_pp = aStar_pp.start()
        #
        #             temp_sum = 0
        #             PR_temp_sum_unknown = 0
        ...
        #                         current_p.x, current_p.y, current_p.z, next_p.x, next_p.y, next_p.z, no_solution_flag,
        #                         PR_temp_sum_known, length_PP, T_plan_optimal))
        #                 aStar = AStar(occ_grid, grid,  current_p, next_p,
        #                               T_plan, threat_list, 1, T_plan_optimal, privacy_radius)
        #                 trajectory_optimal = aStar.start()
        #             else:
        #                 trajectory_optimal = copy.deepcopy(trajectory_optimal_pp)
        #
